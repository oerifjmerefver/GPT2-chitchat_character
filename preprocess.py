...
def preprocess():
    ...
    parser.add_argument('--model_config', default='config/config.json', type=str, required=False, help='tokenize的训练数据集')
    ...
    with open(args.save_path, "w", encoding="utf-8") as f:
        for index, dialogue in enumerate(tqdm(train_data)):
            ...
            input_ids = tokenizer.encode_utterances(utterances)
    ...
    with open("config/config.json", encoding='utf-8', mode='r') as f:
        config_json = json.load(f)
    config_json['vocab_size'] = tokenizer.vocab_size

    with open("config/config.json", encoding='utf-8', mode='w') as f:
        config_json = json.dump(config_json, f, indent='\t')
